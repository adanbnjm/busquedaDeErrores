...
export const obtenerEstudiantes = async (): Promise<Estudiante[]> => {
  const result = await pool.query(
    "SELECT * FROM estudiantes ORDER BY edad DESC, id ASC",
  );
...
export const buscarEstudiantesPorNombre = async (
  nombre: string,
): Promise<Estudiante[]> => {
  const result = await pool.query(
...
export const eliminarEstudiante = async (id: number): Promise<boolean> => {
  const result = await pool.query("DELETE FROM estudiantes WHERE id = $1", [
    id,
  ]);
